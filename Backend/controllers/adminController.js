...
const addTask = async (req, res) => {
    try {
        const { taskName, taskDescription, endDate, assignedMembers, adminId } = req.body;
        const csvFile = req.file;

        if (!taskName || !taskDescription || !endDate || !assignedMembers) {
            return res.json({ success: false, message: "Please fill all required fields." });
        }

        const assignedUsers = JSON.parse(assignedMembers);
        ...
        const distributedUsers = assignedUsers.map((user, index) => {
...
export { loginAdmin, signupAdmin, getAllUsers, addTask, getAllTasks, csvDownloader, getAllMembersForTask };